...
function checkArray_test () {
  ...
  // To add: assert("" == checkArray("m", undefined))

  // Pass type 1.
  assert(checkArray(null, 1) == '') // Won't pass schema check
  assert(checkArray('m', 1) == '') // Won't pass schema check
  assert(checkArray('m', [1]) == '')
  assert(checkArray('m', 3) == '') // Won't pass schema check
  ...
  // Pass type 3.
  assert(checkArray(['m', 'm', 'm'], 3) === '') // Won't pass schema check
  assert(checkArray(['m', 'm', 'm'], [3]) === '')

  // Pass type 4.
  assert(checkArray([['m', 'm', 'm'], ['s', 's', 's']], [2, 3]) === '')
  assert(checkArray([['m', 'm', 'm', null], ['m', 'm', null, 'km']], [2, 4]) == '') // Not valid for label.
  ...
  // Fail type 1.
  emsg = "If not an array, 'units' must be one of: string+,null"
  ...
  let aox = [n, [n], [n, n]]
  let ax = [aox, aox]
  assert(checkArray([ax, ax], [2, 2, 2, 2]) === 'Error in units[0]')
